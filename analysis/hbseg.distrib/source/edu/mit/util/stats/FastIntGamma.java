...
public class FastIntGamma implements FastGamma {
    ...
    public double logGamma(final double p_in){
        double out=0;
        double in = p_in - offset;
        int i_in = (int) in;
        if (in != i_in || in >= log_gamma_cache.length ) out= Gamma.logGamma(p_in);
        else {
            if (!has_log_gamma[i_in]) {
                log_gamma_cache[i_in] = Gamma.logGamma(in+offset);
                has_log_gamma[i_in] = true;
            } 
            out = log_gamma_cache[i_in];
        }
        return out;
    }
    public double gamma(final double p_in){
        double out=0;
        double in = p_in - offset;
