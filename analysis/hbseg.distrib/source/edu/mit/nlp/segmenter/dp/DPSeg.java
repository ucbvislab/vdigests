...
public class DPSeg {
    ...
    **/
    public DPSeg(DPDocument[][] docs, int[][] truths){
        ...
        if (m_debug) out.println("Hash start size: "+start_size*2);
        ...
        expected_dur = total_num_sent / total_num_segs;
    }    
    ...
    **/
    public SegResult[] segEM(double[] init_params){
        ...
        for (double d= cur_log_dispersion-5; d <= cur_log_dispersion+5; d+= .5){
            ...
            out.println(computeTotalLL(m_params)+" "+
                               m_params[1]+" "+
                               gradient[1]);
        }
        ...
        do {
            ...
            if (m_debug) out.println(String.format("delta: %.2e, ctr: %d, score: %.2e, params: %s",
    ...
    */
    protected SegResult[] segmentUnknown(double[] params){
        if (m_debug) out.println("segmenting unknown");
        ...
        for (int docctr = 0; docctr < N; docctr++){
            ...
            for (int t = 0; t <= T; t++){
                for (int t2 = 0; t2 < t; t2++){
                    for (int d =0; d < docs.length; d++){
                        seglls[t][t2] += docs[d].segLL(t2+1,t);
                    }
                }
            }
    ...
    */
    protected SegResult[] segmentKnown(double[] params){ 
        ...
        if (m_debug) out.println("Params: "+JacobUtil.formatArray("%.4f"," ",priors)+" "+Math.exp(params[params.length-1]));
        ...
        for (int docctr = 0; docctr < N; docctr++){
            ...
            if (! use_duration) pdur = null;
            //this checks out ok
//             out.print(String.format("%d %d %.3f ",T,K,Math.exp(params[params.length-1])));
//             for (int i = 0; i < pdur.length; i++){
//                 out.print(String.format("%.3f ",pdur[i]));
//             }
//             out.println();
    ...
    **/
    public double computeTotalLL(double[] logpriors){
        //        if (m_debug) out.print("computing LL for "+JacobUtil.formatArray("%.3f"," ",logpriors)+" ");
        double out = 0;
        ...
        for (int docctr = 0; docctr < N; docctr++){
            ...
            //   out.println(String.format("T:%d K:%d",T,K));
            double[] pdur = computePDur(T,(double)T/m_K[docctr],log_dispersion);
            for (int k = 0; k < K-1; k++){
                if (use_duration) out += pdur[m_segs[docctr][k+1] - m_segs[docctr][k]];
                for (int d = 0; d < m_docs[docctr].length; d++){
                    out += m_docs[docctr][d].segLL(m_segs[docctr][k]+1,m_segs[docctr][k+1]);
                    //                    out.println (" "+k+" "+m_docs[docctr][d].segLLExp(m_segs[docctr][k]+1,m_segs[docctr][k+1],logpriors[d]));
                }
            }
            if (use_duration) out += pdur[T-m_segs[docctr][K-1]];
            for (int d = 0; d < m_docs[docctr].length; d++){
                out += m_docs[docctr][d].segLL(m_segs[docctr][K-1]+1,(int)m_docs[docctr][0].T());
                //out.println (" "+(K-1)+" "+m_docs[docctr][d].segLLExp(m_segs[docctr][K-1]+1,(int)m_docs[docctr][0].T(),logpriors[d]));
            }
        }
        //factor in the IG(1,1) prior on the dispersion parameter
        //out += -(ALPHA + 1) * log_dispersion - BETA / Math.exp(log_dispersion);
        //        if (m_debug) out.println(out);        
        return out;
    }
    ...
    **/
    public double[] computeGradient(double[] logpriors){
        //        out.println("computing Grad for "+logpriors[logpriors.length-1]+" = ");
        //out.print("computing Grad for "+logpriors[0]+" = ");
        ...
        for (int docctr = 0; docctr < N; docctr++){
            ...
            for (int k = 0; k < K - 1; k++){
                ...
                for (int d = 0; d < D; d++){
                    out[d] += m_docs[docctr][d].segLLGradient(m_segs[docctr][k]+1,m_segs[docctr][k+1]);
                }
            }
            ...
            for (int d = 0; d < D; d++){
                out[d] += m_docs[docctr][d].segLLGradient(m_segs[docctr][m_K[docctr]-1]+1,(int)m_docs[docctr][0].T());
            }
        }
        ...
        //out.println(out[out.length-1]);
        return out;
    }

    public void setEDur(double[] p_edurs){ //this is one per file
        //edurs has one element per file.  
        //we're just gonna average over it for now
        assert (p_edurs.length == N);
        expected_dur = 0;
        for (int i = 0; i < p_edurs.length; i++){
            expected_dur += p_edurs[i];
        }
        expected_dur /= p_edurs.length;
        if (m_debug) out.println("DPSeg edur: "+expected_dur);
    }
    ...
    **/
    ...
    double[] m_pdur; //current distribution over durations: size T
    double expected_dur;
