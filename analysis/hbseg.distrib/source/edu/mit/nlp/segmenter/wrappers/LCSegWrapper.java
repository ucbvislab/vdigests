...
public class LCSegWrapper extends Segmenter {
    public LCSegWrapper(){ 
    }
    ...
    public List[] segmentTexts(MyTextWrapper[] texts, int[] num_segs){
        ...
        for (int i = 0; i < texts.length; i++){
            try {
                ...
                File tmp = File.createTempFile("lcseg","tmp");
                PrintStream tmp_out = new PrintStream(new FileOutputStream(tmp));
                //this may not work for block windows
                for (int j = 0; j < texts[i].getSentenceCount(); j++){
                    tmp_out.println(texts[i].getSentenceString(j));
                }
                tmp_out.close();

                //segline = segline + " "+texts[i].getTextFilename();
                segline = segline + " "+tmp.getPath();
                //this doesn't seem to work -- maybe i can do this through the Process class
                //segline = String.format("cat %s | %s",texts[i].getTextFilename(),segline);
