 ...
 *  various segmenters.  
 ...
 <pre>
 SegTester -config config -dir dir -suff suff [-init init] [-debug] [-unknown]
...
//todo: swap out Options for JSAP.  This would combine command line and config-file stuff in one nice interface
public class SegTester {
    public static void main(String[] args){
        ...
        options.getSet("eval").addOption("num-times",Options.Separator.BLANK,Options.Multiplicity.ZERO_OR_ONE);
        options.getSet("eval").addOption("print-segs",Options.Multiplicity.ZERO_OR_ONE);
        ...
        try {
            ...
            seg_tester.out = System.out;
            if (optset.isSet("out")){
                seg_tester.out_file = new File(optset.getOption("out").getResultValue(0));
                seg_tester.out = new PrintStream(new FileOutputStream(seg_tester.out_file));
                segmenter.setPrintStream(seg_tester.out);
            }
            ...
            if (optset.getSetName().equals("eval")){
                seg_tester.print_segs = optset.isSet("print-segs");
                seg_tester.out.println(seg_tester.params.getProps());
                seg_tester.loadFiles(optset);
                
                if (segmenter instanceof Initializable &&
                    optset.isSet("init")){
                    init_file_name = optset.getOption("init").getResultValue(0);
                    ((Initializable)segmenter).initSegs(init_file_name);
                }
                int num_times = 1;
                if (optset.isSet("num-times")) 
                    num_times = new Integer(optset.getOption("num-times").getResultValue(0)).intValue();
                seg_tester.eval(segmenter, num_times); //or maybe output results?
            } 
            else { //it's run
                ...
                List[] hyp_segs = segmenter.segmentTexts
                    (new MyTextWrapper[]{text},
                     new int[]{num_segs});
                seg_tester.out.println(hyp_segs[0]);
    ...
    /**
       This is for some AFS wackness where my process gets killed and somehow
       the file is at zero.  I think that if I close it and then open to append, that'll stop happenning.
     */
    protected void seriouslyFlush(Segmenter segmenter) throws Exception {
        if (out_file != null){
            out.close();
            out = new PrintStream(new FileOutputStream(out_file,true)); //this opens for append
    ...
    protected void loadFiles(OptionSet optset){
        ...
        for (int i = 0; i < filenames.length; i++){
            out.println(the_dir+"/"+filenames[i]);
    ...
    **/
    ...
    /**
       ...
       Needs to not count the ones that have "unlabeled" in the name.
    */
    public void eval(Segmenter segmenter, int num_times){
        ...
        if (segmenter instanceof SegDurParametrizable){
            ((SegDurParametrizable)segmenter).setDurationModel(texts);
        }

        for (int n = 0; n < num_times; n++){
            if (segmenter instanceof CuCoSeg) ((CuCoSeg)segmenter).clear(); 
            if (init_file_name != null && segmenter instanceof Initializable)
                ((Initializable)segmenter).initSegs(init_file_name);
            List[] hyp_segs = segmenter.segmentTexts(texts,MyTextWrapper.getNumSegs(texts));
            labeled_count = 0;
            for (int i = 0; i < hyp_segs.length; i++){
                if (print_segs) out.println(texts[i].getTextFilename()+": "+hyp_segs[i]);
                if (! Pattern.matches(".*unlabeled.*",texts[i].getTextFilename())){
                    labeled_count++;
                    ...
                    out.println(String.format("%s %.4f %.4f", texts[i].getTextFilename(), pk_w, wd_w));
                    total_pk_w += pk_w; total_wd_w += wd_w;
                }
            }
            out.println("----------");
            try {
                seriouslyFlush(segmenter);
                //                out = seriouslyFlush(out_file, out);
            } catch (Exception e){
                e.printStackTrace();
            }
        }
        total_pk_w /= (num_times * labeled_count); total_wd_w /= (num_times * labeled_count);
        //todo: variance
        out.println(String.format("%.4f %.4f",total_pk_w,total_wd_w));
    }
    ...
    protected SegTesterParams params;
    protected MyTextWrapper[] texts;
    protected static String para_ending = ".xtra";
    protected PrintStream out;
    protected static String init_file_name = null;
    protected File out_file;
    protected boolean print_segs;
}
