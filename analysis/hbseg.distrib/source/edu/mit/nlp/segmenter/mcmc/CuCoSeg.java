...
public class CuCoSeg extends Segmenter implements Initializable {
    public CuCoSeg(){
        //        initializer = new BayesWrapper();
        //        initializer = new RandomSegmenter();
        init_segs = new HashMap<String,ArrayList>();
        out = System.out;
    }
    public void initialize(String config_filename){
        ...
        try {
            ...
            k_proposal_weight = SegTesterParams.getDoubleProp(props,"proposal-weight",k_proposal_weight);
            use_extra_features = SegTesterParams.getBoolProp(props,"use-extra-features",use_extra_features);
            k_initializer_name = props.getProperty("initializer",k_initializer_name);
            initializer = (Segmenter) Class.forName(k_initializer_name).
    ...
    public void setPrintStream (PrintStream p_out){ out = p_out; }
    ...
    public void initSegs(String segfilename){
        try {
            ...
            while (in.ready()){
                ...
                //build a hash where the filename is the key and the value is an arraylist of segmentation points
                String[] matchout = line.split(": \\[",2);
                if (matchout.length == 2){
                    String seq = "["+matchout[1];
                    if (! init_segs.containsKey(matchout[0])){
                        init_segs.put(matchout[0],JacobUtil.getIntArray(seq));
                        if (debug) System.out.println("adding key = "+matchout[0]+", value= "+JacobUtil.getIntArray(seq));
                    }
                } 
                else {
                    //                    System.out.println("not doing: "+line);
                }
                // else {
//                 }
...
//                     }
//                     phi_b_0 = theta_0;
//                     out.print(String.format("theta_0=%.3e ",theta_0));
//                     if (use_duration) out.print(String.format("dispersion=%.3e",dispersion));
//                     out.println();
...
//                 }
    ...
    **/
    public List[] segmentTexts(MyTextWrapper[] texts, int[] K){
        ...
        List<Integer> cuephrase_idxs = new ArrayList<Integer>();
        if (b_cuephrases != null){
            for (String cuephrase : b_cuephrases){
                cuephrase_idxs.add(cue_lexmap.getStemId(cuephrase));
                //            System.out.println("added: "+cuephrase+" "+cue_lexmap.getWordId(cuephrase)+" "+cue_lexmap.getStemId(cuephrase));
            }
        }
        for (int i = 0; i < D; i++) {
            ...
            //this seems bogus -- should only include the 1st word, not all words.
            if (b_cuephrases != null){
                for (int t = 1; t < cue_words[i].length; t++){
                    if (cue_words[i][t].length > 0 && cuephrase_idxs.contains(cue_words[i][t][0])){
              //           System.out.println("a valid cue word: "+cue_words[i][t][0]+" "+
//                                            cue_lexmap.getStem(cue_words[i][t][0]));
                        cuephrase_proposal[i][t] += k_proposal_weight;
                    }
                }
            }
        }
        segs = new ArrayList[texts.length];

        //do the initial segmentation
        //load from hash if possible
        //else run the initializer
        for (int i = 0; i < texts.length; i++){
            segs[i] = init_segs.get(texts[i].getTextFilename());
            if (segs[i] == null){
                if (debug) System.out.println ("performing "+texts[i].getTextFilename()+" initialization: "+K[i]);
                List[] local_segs = initializer.segmentTexts
                    (new MyTextWrapper[]{texts[i]}, 
                     new int[]{K[i]});
                if (debug) System.out.println(local_segs[0]);
                if (is_windowing_enabled)
                    segs[i] = MinCutSeg.convertSentence2WindowSegmentation(local_segs[0], texts[i]);
                else segs[i] = local_segs[0];
            } else {
                if (debug) System.out.println ("loaded "+texts[i].getTextFilename()+" initialization");
            }
        }

        if (debug){
            out.println("INITIAL SEGMENTATION: ");
            for (int i = 0; i < D; i++){ out.println(texts[i].getTextFilename()+": "+segs[i]); }
        }

        if (! read_params_from_init && initializer instanceof BayesWrapper && ((BayesWrapper)initializer).em_params){
            //set my params to those guys
            double[] params = ((BayesWrapper)initializer).getParams();
            theta_0 = Math.exp(params[0]);
            phi_b_0 = theta_0;
            out.print(String.format("theta_0=%.3e ",theta_0));
            if (use_duration) {
                dispersion = Math.exp(params[params.length-1]);
                out.print(String.format("dispersion=%.3e",dispersion));
            }
            out.println();
        }
        ...
        printStatus(out,-1); //print initial status
        ...
        for (int i = 0; i < k_num_moves; i++){
            ...
            for (int docid = 0; docid < D; docid++){
                ...
                if (careful_debug) {
                    out.print (String.format("Considering seg %d, %d (%d): ",seg,amount,amount+k_max_move));
                    out.println("proposal prob: "+trans_prob);
                }
                ...
                try {                    
                    ...
                    if (careful_debug) out.println("inverse proposal prob: "+inv_trans_prob);
                    double new_log_prob = computeLogProb(d,seg);
                    //double new_log_prob = computeLogProb();
                    if (careful_debug) out.println("likelihood prob: "+Math.exp(new_log_prob-cur_log_prob));
                    p_accept *= Math.exp(new_log_prob - cur_log_prob); //likelihood ratio
                    if (careful_debug) out.print(String.format("%.1f %.1f %.3f ",new_log_prob,cur_log_prob,p_accept));
                    //                    if (careful_debug) out.print(String.format("%.1f %.1f",new_global_log_prob,cur_global_log_prob));
                    ...
                    if (p_accept > 0 && Uniform.staticNextDouble() < p_accept){
                        seg_moves_accepted++;
                        if (careful_debug) out.println(" accepted");
                    } else {
                        updateSegmentation(d,seg,-amount);
                        if (careful_debug) out.println(" rejected");
                    }
                } catch (Exception e){
                    out.println(String.format("Problem with file %s, seg=%d, amount = %d",texts[d].getTextFilename(),seg,amount));
                    e.printStackTrace();
                }
            }
...
//                 if (debug)
//                 out.println(String.format("Entering optimizer with: %.2e %.2e %.2e %f",
//                                                  theta_0, phi_b_0, phi_o_0, computeLogProb()));
            ...
            if (i % k_output_period == 0) printStatus(out,i);
            annealer.update();
        }
        if (is_windowing_enabled){
            for (int d = 0; d < D; d++) {
        ...
        //printStatus(out,k_num_moves);
        printStatus(out,999999);
        return segs;
    }
    ...
    **/
    protected void printStatus(PrintStream out, int i){
        ...
        int D_labeled = 0;
        for (int d = 0; d < D; d++){
            if (!Pattern.matches(".*unlabeled.*",texts[d].getTextFilename())){
                List curseg = null;
        ...
        out.println(String.format
                           ("%d %.3f [%d %.2e %.2f] [%.1e %.1e %.1e] %.4f %.4f",
                            ...
                            total_wd / D_labeled));
        if (use_extra_features){
            assert(speaker_change_counts[1][0] > 0);
    ...
    protected class PriorOptimizer extends LBFGSWrapper {
        ...
        public void scanParams(double[] params){
            for (int i = 0; i < params.length; i++){
                ...
                out.println("Scanning parameter "+i);
                for (double j = orig_val-5; j <= orig_val+5; j += .2){
                    params[i] = j;                        
                    double x = objectiveFunction(params);
                    double dx[] = evaluateGradient(params);
                    out.println(String.format("%.2f %.3f %.3f %.3f",params[i],Math.exp(params[i]),x,dx[i]));
//                     out.println(" B prior: " + dcm_b.getPrior());
                }
                params[i] = orig_val;
            }
        }
...
//         out.print(String.format(" computing log prob %.2f %.2f\n cue log prob = %.1f",
//                                          dcm_b.getPrior(),
//                                          dcm[0].getPrior(),
//                                          output));
        for (int d = 0; d < D; d++){
...
//         if (debug) out.print(String.format(" evaluating %d->%d %d %d ",
...
//         out.println("PRINTING MOVE PROPOSAL "+segpt+" "+prevsegpt+" "+nextsegpt);
        for (int i = 0; i < proposal.length; i++) {
            int t = segpt + i - k_max_move;
            if (t > prevsegpt && t < nextsegpt) { //ensure validity{
                try {
                    //                    out.println(String.format("%d %.3e",t,cuephrase_proposal[doc][t]));
                    proposal[i] = movedist_proposal[i] * cuephrase_proposal[doc][t]; 
                } catch (Exception e){
                    System.err.println(segpt+" "+i+" "+k_max_move+" "+prevsegpt+" "+nextsegpt+" "+cuephrase_proposal[doc].length);
                    e.printStackTrace();
                    System.exit(0);
                }
                //   out.println(" "+movedist_proposal[i]+" "+cuephrase_proposal[doc][segpt+i-k_max_move]);
            }
            //            out.println(i+" "+proposal[i]);
        }
...
//         out.println(JacobUtil.formatArray("%.2f"," ",proposal));
...
//         for (int i = 0; i < proposal.length; i++){
//             out.print(String.format("%.2f ",out.pdf(i+1)));
//         }
//         out.println();
...
//         }
//         out.println(JacobUtil.formatArray("%d"," ",outcomes));
        return out;
    } 
    ...
    **/
    public void updateSegmentation(int doc, int seg, int amount){
        ...
        //        out.println("updating segmentation: "+old_segpt+" "+new_segpt);
        ...
        //out.println("updating segmentation: "+seg+" "+amount+" "+t1+" "+t2);
        //out.println(segs[doc]);
    ...
    **/
    ...
    protected void changeCountsForSentence(int doc, int t,int sign){
        ...
        //        out.println("changing counts for sentence: "+doc+" "+K[doc]+" "+t+" "+sign);
        for (int j = 0; j < K[doc]; j++){
            int segstart = ((Integer)segs[doc].get(j)).intValue();
            //  out.println(String.format(" segment %d starts at %d",j,segstart));
        ...
        if (beginning){
            if (sign>0){
                CountsManager.addToCountsFirst(b_counts,cue_words[doc][t]);
                CountsManager.addToCountsRest(i_counts[doc][seg],topic_words[doc][t]);
            } else {
                CountsManager.subFromCountsFirst(b_counts,cue_words[doc][t]);
                CountsManager.subFromCountsRest(i_counts[doc][seg],topic_words[doc][t]);
            }
//             if (chi_squared_analysis)
//                 CountsManager.addToCountsRest(non_b_counts,cue_words[doc][t], lambda_b * sign);
            if (use_extra_features){
                if (t > 0) {//can't do it for the first sentence
                    ...
                    assert(speaker_change_counts[0][speaker_change_by_sent[doc][t-1]?0:1] > 0);
                }
            } 
        }
        else {
            if (sign > 0){
                CountsManager.addToCounts(i_counts[doc][seg],topic_words[doc][t]);
                if (chi_squared_analysis)
                    CountsManager.addToCountsFirst(non_b_counts,cue_words[doc][t]);
            } else {
                CountsManager.subFromCounts(i_counts[doc][seg],topic_words[doc][t]);
                if (chi_squared_analysis)
                    CountsManager.subFromCountsFirst(non_b_counts,cue_words[doc][t]);
            }
            if (use_extra_features){
                if (afterbeginning){ //if it's the one right after the beginning
                    ...
                    assert(speaker_change_counts[1][speaker_change_by_sent[doc][t-1]?0:1] > 0);
                }
                else {
                    speaker_change_counts[2][speaker_change_by_sent[doc][t-1]?0:1]+=sign; 
                    assert(speaker_change_counts[2][speaker_change_by_sent[doc][t-1]?0:1] > 0);
                }
            }
        }   
    }
    ...
    public void initCounts(){
        ...
        if (careful_debug){
            out.println("B: "+JacobUtil.formatArray("%.0f"," ",b_counts));
            for (int i = 0; i < D; i++){
                for (int j = 0; j < i_counts[i].length; j++){
                    out.println("I["+i+","+j+"]: "+JacobUtil.formatArray("%.0f"," ",i_counts[i][j]));
                }
            }
        }
    }

    public void clear(){ segs = null; }
    ...
    static double k_proposal_weight = 1f;
    ...
    String k_initializer_name = "edu.mit.nlp.segmenter.RandomSegmenter";
    ...
    Annealer annealer;
    PrintStream out;
    HashMap<String,ArrayList> init_segs; //initial segmentation.  filename->seg pts
}
